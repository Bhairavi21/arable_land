d3.json("json/reqOne1.json", function(data) {
  //data.sort(function(a,b){return b.hectares-a.hectares;});
  var pxAboveBar=10;
var svgHeight = 550;
var svgWidth = 1100;
var maxhectares =d3.max(data,function(d){return d.hectares;}); // You can also compute this from the data
...
function animateBarsUp() {
  ...
  convert.y.domain([0, maxhectares]);

  var tip = d3.tip()
    .attr('class', 'd3-tip')
    .offset([-10, 0])
    .html(function(d) {
      return "<strong>Hectares:</strong> <span>" + d.hectares + "</span>";
    })
...
; // Insert an axis inside this node
  ...
  bars.append('rect')
    ...
    .attr({
      y: function (d, i) {
        return convert.y(d.hectares);
      },
      height: function (d, i) {
        return maxHeight - convert.y(d.hectares);
      }
    });
