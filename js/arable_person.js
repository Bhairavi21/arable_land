d3.json("json/reqOne1.json", function(data) {
  //data.sort(function(a,b){return b.person-a.person;});
  var pxAboveBar=10;
var svgHeight = 550;
var svgWidth = 1100;
var maxperson =d3.max(data,function(d){return d.person;}); // You can also compute this from the data
...
function animateBarsUp() {
  ...
  convert.y.domain([0, maxperson]);

  var tip = d3.tip()
    .attr('class', 'd3-tip')
    .offset([-10, 0])
    .html(function(d) {
      return "<strong>Hectares per person:</strong> <span>" + d.person + "</span>";
    })
...
; // Insert an axis inside this node
  ...
  bars.append('rect')
    ...
    .attr({
      y: function (d, i) {
        return convert.y(d.person);
      },
      height: function (d, i) {
        return maxHeight - convert.y(d.person);
      }
    });
